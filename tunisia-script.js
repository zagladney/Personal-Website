// Tunisia gallery functionality (similar to main gallery)
let currentCarouselIndex = 0;

// Get gallery images from global scope (defined in tunisia-data.js)
function getTunisiaImages() {
    if (typeof tunisiaImages !== 'undefined') {
        return tunisiaImages;
    }
    return [];
}

// Initialize gallery
function initTunisiaGallery() {
    const galleryContainer = document.getElementById('tunisia-container');
    if (!galleryContainer) return;
    
    const images = getTunisiaImages();
    ...
    images.forEach((image, index) => {
        ...
        img.src = image.src;
        img.alt = image.alt || `Tunisia photo ${index + 1}`;
        img.loading = 'lazy';
...
function updateCarouselImage() {
    ...
    if (carouselImage) {
        ...
        img.onload = () => {
            ...
            carouselImage.alt = images[currentCarouselIndex].alt || `Tunisia photo ${currentCarouselIndex + 1}`;
...
function updateScrollIndicator() {
    ...
    // Hide if scrolled down or if there's no more content
...
document.addEventListener('DOMContentLoaded', () => {
    function tryInit() {
        ...
        if (images.length > 0) {
            initTunisiaGallery();
            checkURLHash();
            // Check scroll indicator after images load
