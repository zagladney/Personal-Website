// Crust gallery functionality (similar to Tunisia)
let currentCarouselIndex = 0;

// Get gallery images from global scope (defined in crust-data.js)
function getCrustImages() {
    if (typeof crustImages !== 'undefined') {
        return crustImages;
    }
    return [];
}
...
// Initialize gallery
function initCrustGallery() {
    const galleryContainer = document.getElementById('crust-container');
    if (!galleryContainer) return;
    
    const images = getCrustImages();
    ...
    images.forEach((image, index) => {
        ...
        img.src = image.src;
        img.alt = image.alt || `Crust photo ${index + 1}`;
        img.loading = 'lazy';
...
function updateCarouselImage() {
    ...
    if (carouselImage) {
        ...
        img.onload = () => {
            ...
            carouselImage.alt = images[currentCarouselIndex].alt || `Crust photo ${currentCarouselIndex + 1}`;
